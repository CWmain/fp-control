...
func cameraTilt(delta, value_dictionary: Dictionary[String, Vector3], velocity: Vector3) -> void:	
	...
	value_dictionary["angles"].x += forward_tilt
	
	var right_dot = velocity.dot(right)
	var slide_tilt = clampf(right_dot * deg_to_rad(run_roll), deg_to_rad(-max_roll), deg_to_rad(max_roll))
	
	value_dictionary["angles"].z -= slide_tilt
	
	
func fallKick(value_dictionary: Dictionary[String, Vector3]) -> void:
