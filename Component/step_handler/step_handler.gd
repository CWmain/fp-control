class_name StepHandlerComponenet extends Node
...
func handle_step_climbing() -> void:
	...
	for i in player.get_slide_collision_count():
		...
		if _is_vertical_surface(collision):
			...
			if measured_height > MIN_STEP_HEIGHT and measured_height <= step_height:
...
func _get_player_feet_position() -> Vector3:
	...
	feet_pos.y += FEET_ADJUSTED_HEIGHT # Cast above floor
	return feet_pos
